...
| `DOM.Node` / `DOMNode` | `DOMNode.Payload`, `DOMNode` | ProxyKit publishes the full decoded node payload, including ordered `attributeList`. DataKit model and `DOMTreeSnapshot.Node` publish `id`, names/value/type/kind, lookup attributes, ordered attributes, frame/document URL fields, child count, regular children, content document, shadow roots, template content, pseudo elements, pseudo/shadow-root types, visible children projection, and selection-driven `elementStyles`. Detached-root registry and event mutation binding remain planned. | `DOM.Attribute`, `DOMNode.Attribute`, `DOM.Children` / `DOMNode.Children`, `DOM.PseudoType`, `DOM.ShadowRootType` | `Sources/WebInspectorCoreDOMCSS/DOM/DOMProtocol.swift`; `Sources/WebInspectorCoreDOMCSS/DOM/DOMModelTypes.swift`; `Sources/WebInspectorProxyKit/DOM.swift`; `Sources/WebInspectorDataKit/DOMNode.swift`; `Sources/WebInspectorDataKit/DOMTreeController.swift` | Implemented DOM projection expansion needed before W4b tree-render cutover; detached-root/event expansion planned |
...
| W3 — DataKit model/context owners | `WebInspectorContext` owns semantic state, identity maps, apply handlers, and initial fetched results on a selected owner actor | `Sources/WebInspectorDataKit/**`; DataKit tests; contract tests | Current M3/M4 binding subset completed; `DOMTreeController` now carries ordered attributes and recursive content/shadow/template/pseudo projection needed by W4b tree rendering, and `WebInspectorFetchedResultsController` current snapshot/transaction subsets are implemented. CSS mutation, fetched-results sort/predicate/phase, detached-root event binding, and target-commit proof remain planned | DataKit tests; no UI imports; non-MainActor compile/runtime proof; symbol graph denylist |
