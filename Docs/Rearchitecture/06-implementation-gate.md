...
| --- | --- | --- | --- | --- |
| W1 — package graph + ProxyKit shell | `WebInspectorProxyKit` owns native attach, raw transport, target registry, terminal lifecycle | `Package.swift`; `Sources/WebInspectorProxyKit/**`; `Sources/WebInspectorProxyKitTesting/**`; targeted transport/proxy tests | Current product/shell/rename work completed; raw inbound `TransportReceiver` now belongs to `WebInspectorTransport`; ProxyKit has an initial package-private transport command/event backend with route-filtered typed decoding; native attach and target discovery still need to be rebased behind ProxyKit | `swift test`; targeted transport/proxy tests; symbol graph has no public raw envelope in DataKit |
