...
| Network UI | DataKit `NetworkRequest` / `NetworkBody`, `WebInspectorFetchedResultsController`, and UI-owned filtering/projection own the list/detail state | Existing Network UI consumes Core-only `NetworkSession.orderedRequestIDs`, `requestTopologyRevision`, `requestDisplayRevision`, `requestDisplayChanges(after:)`, and optional response-body conventions. DataKit now supplies request/response body role/kind/syntax/text, `canFetchResponseBody`, and `clearNetworkRequests()`; the UI still must move to the fetched-results contract before Core deletion |
