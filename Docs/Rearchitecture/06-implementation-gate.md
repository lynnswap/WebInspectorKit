...
| --- | --- | --- |
| G0 | Final public product/API surface is listed and legacy transport targets are removed as independently importable modules | Implemented 2026-07-05: `Package.swift` exports only `WebInspectorProxyKit`, `WebInspectorProxyKitTesting`, `WebInspectorDataKit`, and `WebInspectorKit`. `WebInspectorTransport` and `WebInspectorNativeTransport` no longer exist as independently importable products, targets, or ProxyKit source folders. Remaining `TransportSession` / `TransportBackend` package-only symbols are ProxyKit protocol-routing implementation details, not public SDK layers. |
...
| G4 | Worker branches are split by owner boundary and write set, with no overlapping primary files | Active 2026-07-05: package-boundary removal, ProxyKit lifecycle/runtime fixes, DataKit DOM/Network lifecycle fixes, and UI interaction/key/undo fixes are separate owner slices. Workers must not introduce raw transport imports outside ProxyKit; DataKit/UI tests should prefer `WebInspectorProxyKitTesting` unless they are explicitly covering ProxyKit routing internals. |
...
| DOM / Inspector | `DOM.inspect`, `Inspector.inspect` | Inspect alias selects protocol node and closes picker state; `WebInspectorContext.apply(.inspect)` selects registered nodes, or requests the current root subtree with `DOM.requestChildNodes(depth: -1)` before resolving collapsed nodes from subsequent `setChildNodes` events, including inspect events that arrive before the root document is applied. ProxyKit normalizes node-subtype `Inspector.inspect(Runtime.RemoteObject)` through `DOM.requestNode` before emitting `DOM.Event.inspect`. DataKit exposes picker enable/disable command state as `WebInspectorContext.isElementPickerEnabled` and restores the selected node highlight after touch picker selection. UI opens ancestors and scrolls when `DOMTreeController` renders the selected node. | DOM selection coordinator + ProxyKit protocol normalization + UI row presentation | `WebInspectorContext.selectedNode`, `isElementPickerEnabled`, selected-node highlight restore, selection-driven `DOMNode.elementStyles`, `DOMTreeController.transactions`, UI ancestor expansion/scroll | Implemented for current-page nodes. Cross-origin frame-owned node highlight remains an explicit upstream-compatible no-op until page-level frame highlight coordination is designed. |
...
| Network | `responseReceived` | `applyResponseReceived` updates response, body placeholders, state, and creates a request when WebKit reports a response for a load whose `requestWillBeSent` was missed during attach/navigation | Network coordinator | `NetworkRequest.url/method/status/mimeType/requestHeaders/responseHeaders/responseBody/state`, `responseReceivedTimestamp`; `WebInspectorFetchedResultsController` insert/update transaction | Implemented in DataKit shell; response-first request creation matches WebKit NetworkManager behavior |
...
| --- | --- | --- | --- | --- | --- |
| `WebInspectorTarget` | `ProtocolTarget.Record` | `id`, `kind`, `frameID`, `isProvisional`; keep capabilities/pause/routing internals package | `WebInspectorTarget.ID`, `FrameID` | `Sources/WebInspectorProxyKit/Transport/ProtocolTypes.swift`; `Sources/WebInspectorProxyKit/WebInspectorTarget.swift` | Implemented current ProxyKit subset |
| `DOM.Node` / `DOMNode` | `DOM.Node`, `DOMNode` | ProxyKit publishes the full decoded node payload, including ordered `attributeList`. DataKit model and `DOMTreeSnapshot.Node` publish `id`, names/value/type/kind, lookup attributes, ordered attributes, frame/document URL fields, child count, regular children, content document, shadow roots, template content, pseudo elements, pseudo/shadow-root types, visible children projection, selector/XPath projection, selection-driven `elementStyles`, and DOM command entry points for child loading, HTML/path copy, highlight, delete, hide highlight, picker toggle, and page reload. Detached-root registry and shadow/pseudo event mutation binding remain planned. Picker selected-node highlight restore is implemented for current-page nodes. | `DOM.Attribute`, `DOMNode.Attribute`, `DOMNode.CopyTextKind`, `DOM.Children` / `DOMNode.Children`, `DOM.PseudoType`, `DOM.ShadowRootType` | `Sources/WebInspectorProxyKit/DOM.swift`; `Sources/WebInspectorDataKit/DOMNode.swift`; `Sources/WebInspectorDataKit/DOMPathBuilder.swift`; `Sources/WebInspectorDataKit/DOMTreeController.swift`; `Sources/WebInspectorDataKit/WebInspectorContext.swift` | Implemented DOM projection, command/path contract, and current-page highlight restore; detached-root and shadow/pseudo event expansion planned |
...
| `Runtime.ExecutionContext` / `RuntimeContext` | `Runtime.ExecutionContext`, `RuntimeContext.Record` | `id`, `name`, `frameID`, `kind/type`; keep target/runtime-agent IDs package | `Runtime.ContextKind`, `FrameID` | `Sources/WebInspectorProxyKit/Runtime.swift`; `Sources/WebInspectorProxyKit/Transport/RuntimeContextTypes.swift`; `Sources/WebInspectorDataKit/RuntimeContext.swift` | Implemented current subset |
...
| `RawEvent` | `ProtocolEvent` | `domain`, `method`, `params` | none beyond `Data` | `Sources/WebInspectorProxyKit/Transport/TransportTypes.swift`; `Sources/WebInspectorProxyKit/RawEvent.swift` | ProxyKit forward-compat public surface; banned from DataKit public graph |
...
| --- | --- | --- | --- |
| Native page handle | `NativeInspectablePage`, `NativeInspectorBackend`, `WebInspectorNativeBridge` | `@MainActor` | Owns `WKWebView`, `isInspectable`, reload, attach/send/detach, and private controller calls inside `WebInspectorProxyKit`'s native attachment implementation. Do not use `@unchecked Sendable`, `@preconcurrency`, or `nonisolated(unsafe)` to bypass this boundary. |
...
| --- | --- | --- | --- | --- |
| W1 — package graph + ProxyKit shell | `WebInspectorProxyKit` owns native attach, raw protocol routing, target registry, terminal lifecycle | `Package.swift`; `Sources/WebInspectorProxyKit/**`; `Sources/WebInspectorProxyKitTesting/**`; targeted proxy/transport-routing tests | Current product/shell/rename work completed; ProxyKit has a live `WKWebView` native attachment composition root, a package-private live proxy backend with route-filtered typed decoding, materializes current-page semantic targets from `TransportSession`, and routes current-page commands through the current target. Raw transport/native attachment names are no longer package targets or source-folder owners. | `swift test`; targeted proxy/transport-routing tests; symbol graph has no public raw envelope in DataKit |
...
| --- | --- | --- |
| Session attach | `WebInspectorSession` owns one `WebInspectorContainer` and its MainActor `WebInspectorContext`; native attachment uses one `NativeInspectorConnection` / `TransportSession` | Closed for Network and DOM tree/menu/navigation. `WebInspectorSession` still stores `InspectorSession` for DOM element/CSS/runtime Core-backed UI paths; those paths must move before public context handoff |
...
| DOM UI | DataKit `DOMNode`, `CSSStyles`, and `DOMTreeController` own semantic tree/style state; UI owns expansion, selection presentation, and TextKit/render diff only | Closed for DOM tree/menu/navigation: `DOMTreeViewController`, `DOMTreeTextView`, `DOMTreeMenuModel`, and `DOMNavigationItems` read DataKit `WebInspectorContext` / `DOMTreeController` instead of `AttachedInspection.dom`. Remaining Core-only DOM UI gaps are element detail/CSS styles/property mutation and any style refresh behavior still owned by `DOMElementViewController` |
| Fixtures/tests | ProxyKitTesting/DataKit fake backends own preview/test data flow | Network and DOM tree/menu/navigation previews/tests now seed DataKit context data. DOM element/CSS previews/tests still build Core sessions and must move with the element-detail cutover |
