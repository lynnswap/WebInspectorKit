...
| G3 | Story A/A2/B/C contract-test package shape is defined with validation commands | Implemented as standalone `ContractTests/` package over public products; Story B custom tabs compile against `WebInspectorSession`. Runtime DataKit access from custom tabs remains intentionally unshipped until W4 replaces the built-in UI's legacy Core source of truth with the same session-owned `WebInspectorContext`; do not close this by adding `session.modelContext` as a side channel. |
...
| `NetworkRequest` | `NetworkRequest` | M3/W4 publishes URL/method/resource type/state/status/mime/source-map, headers, request/response/data/terminal timestamps, decoded/encoded length, metrics, redirects, websocket state, request body, response body, and `canFetchResponseBody`. `documentURL`, cached body metadata, `responseSource`, `initiator`, and wall-time/detail metadata are planned expansion. | `Network.ResourceType`, `NetworkBody`, `RedirectHop`, `WebSocketState` | `Sources/WebInspectorCoreConsoleNetwork/Network/NetworkModel.swift`; `Sources/WebInspectorDataKit/NetworkRequest.swift` | Implemented current subset; detail expansion planned |
...
| Network requests/body | `NetworkSession`, private `NetworkRequestStore`, `NetworkRequest`, and `NetworkBody` in `Sources/WebInspectorCoreConsoleNetwork/Network` own request order, state, metrics, and body fetch phase | Network coordinator, request identity/order, request/response/body/websocket lifecycle, request/response body role/kind/syntax/text, and response body fetch availability | Cached body metadata, response source, initiator, wall-time/detail metadata |
...
| --- | --- | --- |
| DOM identity | Fake proxy emits document and child mutations; `rootNode`, `node(for:)`, repeated payload application, and `DOMTreeController.snapshot` / `transactions` keep stable semantic node identity and current tree state | Detached-root, shadow-root, pseudo-element, and content-document expansion |
| Network request/body lifecycle | `requestWillBeSent` → `responseReceived` → `dataReceived` → terminal event → `fetchResponseBody()` produces one ordered request, request-body projection, response-body hints, expected body phase, and fetch availability | Cached metadata, initiator, and detail expansion |
...
| W3 — DataKit model/context owners | `WebInspectorContext` owns semantic state, identity maps, apply handlers, and initial fetched results on a selected owner actor | `Sources/WebInspectorDataKit/**`; DataKit tests; contract tests | Current M3/M4 binding subset completed; `DOMTreeController` and `WebInspectorFetchedResultsController` current snapshot/transaction subsets implemented; richer DOM/CSS/Network fields, fetched-results sort/predicate/phase, and target-commit proof remain planned | DataKit tests; no UI imports; non-MainActor compile/runtime proof; symbol graph denylist |
| W4 — UIKit compatibility layer | `WebInspectorSession` owns the MainActor/UIKit facade over one DataKit container/context; built-in tabs and app tabs keep receiving session | `Sources/WebInspectorUI/**`, `Sources/WebInspectorKit/**`, UIKit tests, Monocly call site if custom tab is included | Pending. The current `WebInspectorSession` facade is preserved for compile compatibility, but built-in UI is still backed by legacy Core state. W4 is a cutover, not an accessor patch: it must move built-in UI reads to the session-owned `WebInspectorContext` before publishing any DataKit handoff to custom tabs. Adding `session.modelContext` or another public context side channel while `AttachedInspection` remains the UI source is prohibited. | default workspace `xcodebuild test`; W4-specific UIKit tests must prove one context powers built-in and custom tabs; Monocly build/test if changed |
...
| --- | --- | --- | --- |
| W4a — session attachment cutover | `WebInspectorSession` owns one `WebInspectorContainer` / `WebInspectorContext` for an attachment | `Sources/WebInspectorUI/Containers/WebInspectorSession.swift`; `Sources/WebInspectorKit/**`; focused parent/container tests | `session.attach(to:)` creates the DataKit graph used by UIKit without a second `WKWebView` inspector connection. No public custom-tab handoff yet. |
...
| Network UI | DataKit `NetworkRequest` / `NetworkBody`, `WebInspectorFetchedResultsController`, and UI-owned filtering/projection own the list/detail state | Existing Network UI consumes Core-only `NetworkSession.orderedRequestIDs`, `requestTopologyRevision`, `requestDisplayRevision`, `requestDisplayChanges(after:)`, `reset()`, and optional response-body conventions. DataKit now supplies request/response body role/kind/syntax/text and `canFetchResponseBody`; the UI still must move to the fetched-results contract or DataKit clear/log actions before Core deletion |
