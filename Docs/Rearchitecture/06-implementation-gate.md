...
test -n "$graph"
! rg 'Network\.Request|Network\.Response|WebInspectorSortOrder|WebInspectorSortDescriptor|WebInspectorFetchPredicate|WebInspectorDataPhase|WebInspectorModelActor|WebInspectorModelExecutor|WebInspectorTargetChanges|RawEvent|\bWebView[A-Za-z0-9_]*|WebViewKit|@WebView' "$graph"
```
...
| W4e — legacy UI scaffolding deletion | `AttachedInspection` / `InspectorSession` no longer back the UIKit product | `Package.swift`; `Sources/WebInspectorUI/**`; `Sources/WebInspectorKit/**`; obsolete UI/Core tests/docs | Remaining Core targets are either deleted, test-only characterization moved down, or explicitly retained for non-UI internals with no public product path. |
