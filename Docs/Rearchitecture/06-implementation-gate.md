...
| --- | --- | --- | --- | --- |
| W1 — package graph + ProxyKit shell | `WebInspectorProxyKit` owns native attach, raw transport, target registry, terminal lifecycle | `Package.swift`; `Sources/WebInspectorProxyKit/**`; `Sources/WebInspectorProxyKitTesting/**`; targeted transport/proxy tests | Current product/shell/rename work completed; raw inbound `TransportReceiver` now belongs to `WebInspectorTransport`; ProxyKit has a package-private transport command/event backend with route-filtered typed decoding, materializes current-page semantic targets from `TransportSession`, and routes current-page commands through the transport-owned current target; native attach still needs to be rebased behind ProxyKit | `swift test`; targeted transport/proxy tests; symbol graph has no public raw envelope in DataKit |
