...
- `DOMModel.swift` は state/component ファイル分割後も private helper が多い。selector path、snapshot、frame projection matching の owner split は実施済みだが、remaining mutation helper を切る場合は可視性を広げる前に owner 境界を再確認する。
...
- DOM delete/undo queue owner 化では、WebKit の `DOMTreeElement.remove()` を確認した。WebKit は `DOMNode.removeNode` 成功 callback 後に tree element を外す。WebInspectorKit は UndoManager の undo/redo を async protocol command として順序実行する差分があるため、tail task と task dictionary を別々に同期するのではなく、`QueuedOperation(id, generation, task)` が queue membership / cancellation / finish を一体で所有する形にした。
...
- `NetworkDetailViewController` の response body fetch observation token、weak observed request、debug delivery token を `NetworkResponseBodyFetchObservationBinding` に集約。detail view は preview role/visibility の判定と model fetch 呼び出しに集中し、observation lifetime と stale request rejection は binding owner が持つ形にした。
...
- `NetworkBody` の text representation preparation は generation、prepared generation、task、batch/deferred invalidation flag の分散保持を `NetworkBodyTextRepresentationPreparationState` に集約。content/hint 更新時の invalidation、detached pretty-print task の current-generation 判定、task cancel/finish は state owner が持ち、`NetworkBody` 本体は display text/syntax の observable surface を維持する形にした。
...
- `TransportSession` の current main page target waiter ID 採番と waiter dictionary を `TransportMainPageTargetWaiterStore` に集約。timeout/cancel/detach/target event 後の promise fulfill ordering は actor 本体に残し、waiter membership と take-all/remove の不変条件だけを store owner に閉じた。
...
- `FrameDocumentProjectionResolver` を追加し、frame document の owner document 解決、既存 owner 維持判定、frameID 優先 matching、URL fallback、ambiguous/pending 決定を DOM mutation owner から分離。`DOMSession` は resolver の `attach` / `detach` decision を `FrameDocumentProjectionIndex` に反映するだけにした。親チェーン接続判定は `DOMDocumentState.containsConnectedNode` に移し、document/node index owner の query として DOM event visibility と projection matching が共有する形にした。
...
- `CSSSession` の selected identity と unavailable reason の分散保持を `CSSSelectedNodeStylesSelection` に集約。observable surface の `selectedNodeStyles` は stored property のまま残し、selection owner が current identity、unavailable reason、target removal、stale refresh failure、current styles unavailable の遷移を持つ形にした。既存の `selectedNodeStyles` 再 publish 抑制契約は維持した。
...
- `swift test --filter NetworkModelTests -Xswiftc -strict-concurrency=minimal` (2026-06-13、Network body text preparation state owner 化後 green)
- `swift test --filter WebInspectorCoreTests -Xswiftc -strict-concurrency=minimal` (2026-06-13、Network body text preparation state owner 化後 green)
- `xcodebuild test -workspace WebInspectorKit.xcworkspace -scheme WebInspectorKit -destination 'platform=iOS Simulator,name=iPhone 17,OS=latest' -only-testing:WebInspectorUITests/NetworkDetailViewControllerTests` (2026-06-13、Network body text preparation state owner 化後 green)
- `swift test -Xswiftc -strict-concurrency=minimal` (2026-06-13、Network body text preparation state owner 化後 green)
- `xcodebuild test -workspace WebInspectorKit.xcworkspace -scheme WebInspectorKit -destination 'platform=iOS Simulator,name=iPhone 17,OS=latest'` (2026-06-13、Network body text preparation state owner 化後 green)
...
- `swift test --filter CSSModelTests -Xswiftc -strict-concurrency=minimal` (2026-06-13、CSS selected node styles selection owner 化後 green)
- `swift test --filter CSSProtocolDispatchingTests -Xswiftc -strict-concurrency=minimal` (2026-06-13、CSS selected node styles selection owner 化後 green)
- `swift test --filter WebInspectorCoreTests -Xswiftc -strict-concurrency=minimal` (2026-06-13、CSS selected node styles selection owner 化後 green)
- `swift test -Xswiftc -strict-concurrency=minimal` (2026-06-13、CSS selected node styles selection owner 化後 green)
- `xcodebuild test -workspace WebInspectorKit.xcworkspace -scheme WebInspectorKit -destination 'platform=iOS Simulator,name=iPhone 17,OS=latest'` (2026-06-13、CSS selected node styles selection owner 化後 green)
...
- `xcodebuild test -project Monocly/Monocly.xcodeproj -scheme Monocly -destination 'platform=iOS Simulator,name=iPhone 17,OS=latest' -only-testing:MonoclyTests/MonoclyLifecycleTests` (2026-06-13、Monocly inspector scene session record 化後 green)
...
- `swift test --filter WebInspectorTransportTests -Xswiftc -strict-concurrency=minimal` (2026-06-13、Transport main page target waiter store owner 化後 green)
- `swift test -Xswiftc -strict-concurrency=minimal` (2026-06-13、Transport main page target waiter store owner 化後 green)
- `xcodebuild test -workspace WebInspectorKit.xcworkspace -scheme WebInspectorKit -destination 'platform=iOS Simulator,name=iPhone 17,OS=latest'` (2026-06-13、Transport main page target waiter store owner 化後 green)
...
- `swift test --filter DOMModelTests -Xswiftc -strict-concurrency=minimal` (2026-06-13、DOMPathBuilder owner 化後 green)
...
- `swift test --filter DOMModelTests -Xswiftc -strict-concurrency=minimal` (2026-06-13、FrameDocumentProjectionResolver owner 化後 green)
- `swift test --filter WebInspectorCoreTests -Xswiftc -strict-concurrency=minimal` (2026-06-13、FrameDocumentProjectionResolver owner 化後 green)
...
- `xcodebuild test -workspace WebInspectorKit.xcworkspace -scheme WebInspectorKit -destination 'platform=iOS Simulator,name=iPhone 17,OS=latest' -only-testing:WebInspectorUITests/NetworkDetailViewControllerTests` (2026-06-13、Network response body fetch observation binding owner 化後 green)
- `xcodebuild test -workspace WebInspectorKit.xcworkspace -scheme WebInspectorKit -destination 'platform=iOS Simulator,name=iPhone 17,OS=latest'` (2026-06-13、Network response body fetch observation binding owner 化後 green)
