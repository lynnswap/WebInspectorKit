...
function loadTypeScript() {
  try {
    ...
    console.error("Missing typescript. Run: (cd ObfuscateJS && pnpm install)");
    process.exit(1);
  }
}
...
function loadEsbuild() {
  try {
    ...
    console.error("Missing esbuild. Run: (cd ObfuscateJS && pnpm install)");
    process.exit(1);
  }
}
...
async function main() {
  ...
  if (shouldObfuscate) {
    try {
      ...
      console.error("Missing terser. Run: (cd ObfuscateJS && pnpm install)");
      process.exit(1);
    }
  }
