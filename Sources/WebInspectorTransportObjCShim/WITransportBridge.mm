...
static BOOL controllerCandidateAtOffset(void *pageProxy, ptrdiff_t offset, void **controllerOut, void **frontendRouterOut, void **backendDispatcherOut)
{
    ...
    if (!frontendRouterPointer(controller, &frontendRouter) || !frontendRouter || !pointerIsWritableMapped(frontendRouter))
        return NO;
#else
    frontendRouterPointer(controller, &frontendRouter);
    if (frontendRouter && !pointerIsWritableMapped(frontendRouter))
        return NO;
#endif
                              ...
                              error:(NSError * _Nullable __autoreleasing *)error
{
    ...
    if ((requiresInspectorConnection && !_inspector)
        || !_controller
        || !_backendDispatcher
        || (resolvedFunctions.frontendAttachmentStrategy == WITransportBridgePrivate::FrontendAttachmentStrategyFrontendRouter && !frontendRouter)
        || (resolvedFunctions.frontendAttachmentStrategy == WITransportBridgePrivate::FrontendAttachmentStrategyFrontendRouter && !didResolveTargetAgent)) {
