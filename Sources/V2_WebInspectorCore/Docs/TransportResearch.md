...
This means the V2 transport needs to capture target/frame/execution-context facts at the protocol envelope boundary and pass them to domain models as already-resolved semantic ownership.

## Domain Routing Categories

V2 transport should route by protocol domain category, not by a blanket "send every command to the selected node's target" rule.
...
- Domain support is capability based. V2 should not assume that every target exposes every domain.
...
V2 should preserve the good boundary from WebInspectorUI, but avoid inheriting its URL-based frame document splice fallback.
...
- V2's frontend-facing `NetworkRequest.ID` can remain `targetID + requestID`, but transport must be ready to carry a separate backend resource identity for body/certificate/lazy fetch commands.
