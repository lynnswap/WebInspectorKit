...
source and captured V2 logs from cross-origin iframe pages.
...
- WebInspectorUI also does not synchronously reload the page document inside
  ...
  hook. V2 logs that show `documentUpdated.reload.start` followed by
  `model.replaceDocumentRoot` are evidence of a V2-specific race: a new shallow
...
- `InspectorDOMAgent` only emits mutation events for nodes it has already bound
  for the frontend, but the meaning of V2 `missingParent` depends on the event:
  - `childNodeInserted`: backend only sends this when the parent is bound and
    ...
    V2 target/document/node mirror invariant breach.
...
- The current WebInspectorUI frame DOM code is not a complete, copy-and-paste
  answer for V2. It is guarded by target capability metadata, relies on a
  fragile URL fallback to find iframe owners, and still has FIXME markers for
  routing most frame-target DOM mutation events. V2 should match the invariants
...
not a blanket source-level template. V2 should intentionally split WebKit
...
Design V2 independently where upstream WebInspectorUI is explicitly
...
- ad iframe refresh and provisional frame commit behavior: upstream has target
  commit plumbing, but V2 still needs explicit generation/projection state to
  avoid corrupting the parent page DOM.

The balance is: match WebKit's proven boundaries, but do not inherit WebKit's
temporary URL fallback or missing frame-event handlers as V2 architecture. When
the upstream code says FIXME, V2 should model the intended final invariant and
...
frame targets through `FrameDOMAgent`. V2 should preserve that split instead
...
ancestor and then emits `setChildNodes` for real parent ids. Therefore V2 must
...
The upstream FIXME around `_trySpliceFrameDocumentIntoNode` is central to V2:
...
- It is order-sensitive: current WebInspectorUI attaches the first matching
  node. V2 should not copy that as-is. A URL fallback should attach only when
...
Therefore V2 needs an explicit model slot for future correct data even if the
...
The correctness baseline is conservative: if V2 cannot resolve the owner with
