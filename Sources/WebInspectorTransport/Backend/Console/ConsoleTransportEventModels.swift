    ...
    enum JSONValue: Decodable, Sendable {
        ...
        package var summary: String {
            ...
            case .string(let stringValue):
                return escapedJSONStringLiteral(stringValue)
            case .number(let numericValue):
                if numericValue.rounded() == numericValue,
                   let exactInteger = Int(exactly: numericValue) {
        ...
        private func escapedJSONStringLiteral(_ stringValue: String) -> String {
            if let data = try? JSONEncoder().encode(stringValue),
