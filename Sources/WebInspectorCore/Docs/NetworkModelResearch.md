...
This note records the WebKit Network model behavior that `WebInspectorCore` is intentionally matching.
...
`redirectIndex` never goes back to WebKit as a protocol request identity. It is a WebInspector UI/HAR/history identifier only.

## Current Coverage

Implemented in the WebInspector model:
