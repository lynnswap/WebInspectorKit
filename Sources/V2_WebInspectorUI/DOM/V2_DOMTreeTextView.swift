...
@MainActor
final class V2_DOMTreeTextView: UIScrollView, @preconcurrency NSTextViewportLayoutControllerDelegate, UITextInput, UITextInteractionDelegate {
    ...
    private let textContentView = V2_DOMTreeTextContentView()
    private let fragmentViewMap = NSMapTable<NSTextLayoutFragment, V2_DOMTreeTextLayoutFragmentView>.weakToWeakObjects()
    private var lastUsedFragmentViews: Set<V2_DOMTreeTextLayoutFragmentView> = []
    private lazy var findCoordinator = V2_DOMTreeFindCoordinator(textView: self)
    ...
    private var textStorage: NSTextStorage {
        guard let storage = textContentStorage.textStorage else {
            fatalError("V2_DOMTreeTextView requires NSTextContentStorage-backed NSTextStorage")
        }
        return storage
    }
    ...
    override var keyCommands: [UIKeyCommand]? {
        [
            UIKeyCommand(
                title: v2WILocalized("dom.tree.extend_selection_up", default: "Extend Selection Up"),
                action: #selector(extendMultiSelectionUp),
                input: UIKeyCommand.inputUpArrow,
                modifierFlags: .shift,
                discoverabilityTitle: v2WILocalized("dom.tree.extend_selection_up", default: "Extend Selection Up")
            ),
            UIKeyCommand(
                title: v2WILocalized("dom.tree.extend_selection_down", default: "Extend Selection Down"),
                action: #selector(extendMultiSelectionDown),
                input: UIKeyCommand.inputDownArrow,
                modifierFlags: .shift,
                discoverabilityTitle: v2WILocalized("dom.tree.extend_selection_down", default: "Extend Selection Down")
            ),
            UIKeyCommand(
                title: v2WILocalized("dom.tree.select_all", default: "Select All"),
                action: #selector(selectAllRenderedRows),
                input: "a",
                modifierFlags: .command,
                discoverabilityTitle: v2WILocalized("dom.tree.select_all", default: "Select All")
            ),
            UIKeyCommand(
                title: v2WILocalized("dom.tree.find", default: "Find"),
                action: #selector(showFindNavigator),
                input: "f",
                modifierFlags: .command,
                discoverabilityTitle: v2WILocalized("dom.tree.find", default: "Find")
            )
        ]
    }
    ...
    func textViewportLayoutControllerWillLayout(_ textViewportLayoutController: NSTextViewportLayoutController) {
        lastUsedFragmentViews = Set(textContentView.subviews.compactMap { $0 as? V2_DOMTreeTextLayoutFragmentView })
    }
    ...
    func textViewportLayoutController(
        ...
        let fragmentView: V2_DOMTreeTextLayoutFragmentView
        if let cachedView = fragmentViewMap.object(forKey: textLayoutFragment) {
            ...
            fragmentView = V2_DOMTreeTextLayoutFragmentView(layoutFragment: textLayoutFragment, frame: surfaceFrame)
    ...
    override func copy(_ sender: Any?) {
        guard selectedTextNSRange.length > 0,
              let text = text(in: V2_DOMTreeTextRange(range: selectedTextNSRange))
        else {
            return
        }
    ...
    private func configureInteractions() {
        ...
        registerForTraitChanges(UITraitCollection.systemTraitsAffectingColorAppearance) { (self: V2_DOMTreeTextView, _) in
    ...
    private func makeTextSelectionEditMenu(for range: NSRange) -> UIMenu {
        ...
        let copyText = UIAction(
            ...
            guard let self,
                  let text = self.text(in: V2_DOMTreeTextRange(range: range)),
                  !text.isEmpty
            else {
                return
            }
    ...
    private func reapplyTextAttributes() {
        ...
        for case let fragmentView as V2_DOMTreeTextLayoutFragmentView in textContentView.subviews {
            fragmentView.setNeedsDisplay()
        }
    }
    ...
    private func configureHighlights(
        for fragmentView: V2_DOMTreeTextLayoutFragmentView,
        surfaceFrame: CGRect
    ) {
...
extension V2_DOMTreeTextView {
    ...
    var selectedTextRange: UITextRange? {
        get { V2_DOMTreeTextRange(range: selectedTextNSRange) }
        set { setSelectedTextRange(newValue) }
    }

    var markedTextRange: UITextRange? {
        markedTextNSRange.map(V2_DOMTreeTextRange.init(range:))
    }
    ...
    var beginningOfDocument: UITextPosition {
        V2_DOMTreeTextPosition(offset: 0)
    }

    var endOfDocument: UITextPosition {
        V2_DOMTreeTextPosition(offset: renderedTextUTF16Length)
    }
    ...
    func textRange(from fromPosition: UITextPosition, to toPosition: UITextPosition) -> UITextRange? {
        ...
        return V2_DOMTreeTextRange(range: clampedTextRange(NSRange(location: lower, length: upper - lower)))
    }

    func position(from position: UITextPosition, offset: Int) -> UITextPosition? {
        V2_DOMTreeTextPosition(offset: clampedTextOffset(self.offset(from: position) + offset))
    }
    ...
    func position(within range: UITextRange, farthestIn direction: UITextLayoutDirection) -> UITextPosition? {
        ...
        return V2_DOMTreeTextPosition(offset: clampedTextOffset(offset))
    }
    ...
    func characterRange(byExtending position: UITextPosition, in direction: UITextLayoutDirection) -> UITextRange? {
        ...
        return V2_DOMTreeTextRange(range: clampedTextRange(range))
    }
    ...
    func selectionRects(for range: UITextRange) -> [UITextSelectionRect] {
        ...
        if let lastRect = rects.last as? V2_DOMTreeTextSelectionRect {
    ...
    func closestPosition(to point: CGPoint) -> UITextPosition? {
        V2_DOMTreeTextPosition(offset: textOffset(at: point))
    }

    func closestPosition(to point: CGPoint, within range: UITextRange) -> UITextPosition? {
        let range = clampedTextRange(nsRange(from: range))
        return V2_DOMTreeTextPosition(offset: min(max(textOffset(at: point), range.location), NSMaxRange(range)))
    }

    func characterRange(at point: CGPoint) -> UITextRange? {
        let offset = textOffset(at: point)
        return V2_DOMTreeTextRange(
    ...
    private func clearTextSelection() {
        setSelectedTextRange(V2_DOMTreeTextRange(range: NSRange(location: 0, length: 0)))
    }
    ...
    private func nsRange(from range: UITextRange) -> NSRange {
        guard let range = range as? V2_DOMTreeTextRange else {
    ...
    private func offset(from position: UITextPosition) -> Int {
        guard let position = position as? V2_DOMTreeTextPosition else {
            return 0
        }
...
extension V2_DOMTreeTextView {
    ...
    func selectTextForTesting(_ text: String) {
        ...
        setSelectedTextRange(V2_DOMTreeTextRange(range: range))
    }
    ...
    func selectTextForTesting(from startText: String, through endText: String) {
        ...
        setSelectedTextRange(V2_DOMTreeTextRange(range: NSRange(location: lowerBound, length: upperBound - lowerBound)))
    }

    var selectedTextForTesting: String {
        text(in: V2_DOMTreeTextRange(range: selectedTextNSRange)) ?? ""
    }
    ...
    func editMenuTitlesForSelectedTextForTesting() -> [String] {
        ...
        return editMenu(
            for: V2_DOMTreeTextRange(range: selectedTextNSRange),
    ...
    func decorateFindTextForTesting(query: String) {
        clearFindDecorations()
        for range in V2_DOMTreeFindCoordinator.searchRanges(in: renderedText, queryString: query) {
            decorateFindTextRange(range, style: .found)
        }
        if let firstRange = V2_DOMTreeFindCoordinator.searchRanges(in: renderedText, queryString: query).first {
    ...
    var fragmentSubviewCountForTesting: Int {
        textContentView.subviews.filter { $0 is V2_DOMTreeTextLayoutFragmentView }.count
    }
...
private final class V2_DOMTreeTextRange: UITextRange {
    ...
    override var start: UITextPosition {
        V2_DOMTreeTextPosition(offset: range.location)
    }

    override var end: UITextPosition {
        V2_DOMTreeTextPosition(offset: NSMaxRange(range))
    }
...
private final class V2_DOMTreeTextSelectionRect: UITextSelectionRect {
...
private struct DOMTreeResolvedTextAttributes {
    ...
    init(traitCollection: UITraitCollection) {
        ...
        let theme = DOMTreeHighlightTheme.webInspector
        base = [
            .font: V2_DOMTreeTextView.font,
            .paragraphStyle: V2_DOMTreeTextView.paragraphStyle,
            .baselineOffset: V2_DOMTreeTextView.textBaselineOffset,
...
private final class V2_DOMTreeTextContentView: UIView {
...
private final class V2_DOMTreeTextLayoutFragmentView: UIView {
