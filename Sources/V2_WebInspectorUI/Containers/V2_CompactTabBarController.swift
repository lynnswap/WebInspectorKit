...
@MainActor
package final class V2_CompactTabBarController: UITabBarController, UITabBarControllerDelegate {
    private let session: V2_WISession
    private let tabTransitionAnimator = V2_NoAnimationTabTransitionAnimator()
    private var nativeTabByItemID: [V2_TabDisplayItem.ID: UITab] = [:]
    private let observationScope = ObservationScope()
    private var isRenderingSelection = false

    package init(session: V2_WISession) {
...
@MainActor
private final class V2_NoAnimationTabTransitionAnimator: NSObject, UIViewControllerAnimatedTransitioning {
