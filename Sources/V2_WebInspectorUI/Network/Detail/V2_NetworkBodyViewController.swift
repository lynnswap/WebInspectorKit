...
final class V2_NetworkBodyViewController: UIViewController {
    ...
    private func renderBody() {
        ...
        guard let body else {
            applyBodyDisplay(
                text: v2WILocalized("network.body.unavailable", default: "Body unavailable"),
        ...
        case .available:
            displayText = v2WILocalized("network.body.available", default: "Body available")
            syntaxKind = .plainText
        case .fetching:
            displayText = v2WILocalized("network.body.fetching", default: "Fetching body...")
            syntaxKind = .plainText
        case .loaded:
            displayText = body.textRepresentation
                ?? v2WILocalized("network.body.unavailable", default: "Body unavailable")
    ...
    private func localizedDescription(for error: NetworkBodyFetchError) -> String {
        switch error {
        case .unavailable:
            v2WILocalized("network.body.fetch.error.unavailable", default: "Body unavailable")
        case .decodeFailed:
            v2WILocalized("network.body.fetch.error.decode_failed", default: "Body decode failed")
        case .unknown(let message):
            message ?? v2WILocalized("network.body.fetch.error.unknown", default: "Body fetch failed")
        }
    }
