...
final class NetworkFilterMenuController {
    ...
    private var selectedIndicatorUpdateTask: Task<Void, Never>?
    ...
    func sync(
        ...
        let isVisibleMenu = notifyingVisibleMenu && spiRuntime.hasVisibleMenu(for: barButtonItem)

        guard isVisibleMenu else {
            selectedIndicatorUpdateTask?.cancel()
            applySelectedIndicator(shouldSelect)
        ...
        scheduleSelectedIndicatorUpdate(afterVisibleMenuDismisses: shouldSelect)
    }
    ...
    private func applySelectedIndicator(_ shouldSelect: Bool) {
    ...
    private func scheduleSelectedIndicatorUpdate(afterVisibleMenuDismisses shouldSelect: Bool) {
        selectedIndicatorUpdateTask?.cancel()
        selectedIndicatorUpdateTask = Task { @MainActor [weak self] in
            guard let self else {
                return
            }
            for _ in 0..<20 {
                ...
                if spiRuntime.hasVisibleMenu(for: barButtonItem) == false {
                    applySelectedIndicator(shouldSelect)
                    return
                }
                try? await Task.sleep(nanoseconds: 25_000_000)
            }
            // Fallback: apply even if visibility check remains stale.
            applySelectedIndicator(shouldSelect)
        }
    }
