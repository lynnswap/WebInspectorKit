...
@MainActor
final class NetworkEntryDetailViewController: UIViewController, UICollectionViewDelegate {
    ...
    private lazy var modeMenu = NetworkEntryDetailModeMenu(
    ...
    fileprivate var mode: NetworkEntryDetailMode = .overview {
        didSet {
    ...
    private func makeDataSource() -> UICollectionViewDiffableDataSource<SectionIdentifier, ItemIdentifier> {
        let overviewCellRegistration = UICollectionView.CellRegistration<NetworkEntryOverviewCell, NetworkEntry> {
            cell, _, entry in
            cell.bind(entry: entry)
        }
        let fieldCellRegistration = UICollectionView.CellRegistration<NetworkEntryFieldCell, ItemIdentifier> {
    ...
    private func configure(_ cell: NetworkEntryFieldCell, item: ItemIdentifier) {
        guard let selectedEntry else {
            cell.clear()
            return
        }
...
@MainActor
private final class NetworkEntryDetailModeMenu {
    private weak var detailViewController: NetworkEntryDetailViewController?
    private let inspector: WINetworkModel
    private let observationScope = ObservationScope()
    private let selectedEntryObservationScope = ObservationScope()
    ...
    init(detailViewController: NetworkEntryDetailViewController, inspector: WINetworkModel) {
    ...
    private func makeMenu(includesImages: Bool) -> UIMenu {
        UIMenu(
            ...
            children: NetworkEntryDetailMode.allCases.map { mode in
                UIAction(
                    title: mode.title,
    ...
    private func isModeEnabled(_ mode: NetworkEntryDetailMode) -> Bool {
...
#if DEBUG
extension NetworkEntryDetailViewController {
    var collectionViewForTesting: UICollectionView {
        collectionView
    }

    var currentModeForTesting: NetworkEntryDetailMode {
        mode
    }
    ...
    func setModeForTesting(_ mode: NetworkEntryDetailMode) {
        self.mode = mode
    }
}
#endif
...
#Preview("Network Detail") {
    WIUIKitPreviewContainer {
        ...
        return UINavigationController(
            rootViewController: NetworkEntryDetailViewController(inspector: inspector)
        )
    }
}
#endif
