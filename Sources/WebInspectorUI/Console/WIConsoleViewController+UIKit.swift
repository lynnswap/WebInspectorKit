...
public final class WIConsoleViewController: UIViewController, UICollectionViewDelegate {
    ...
    private var pendingSnapshotEntries: [WIConsoleEntry]?
    private var pendingSnapshotStickToBottom = false
    ...
    private func reloadData() {
        ...
        pendingSnapshotEntries = entries
        pendingSnapshotStickToBottom = pendingSnapshotStickToBottom || isScrolledNearBottom

        guard snapshotApplyTask == nil else {
            return
        }

        snapshotApplyTask = Task { @MainActor [weak self] in
            guard let self else {
                return
            }

            while let entries = self.pendingSnapshotEntries {
                let shouldStickToBottom = self.pendingSnapshotStickToBottom
                self.pendingSnapshotEntries = nil
                self.pendingSnapshotStickToBottom = false
                ...
                snapshot.reconfigureItems(entries)
    ...
    var firstRowAccessoryCountForTesting: Int {
        collectionView.layoutIfNeeded()
        return (collectionView.cellForItem(at: IndexPath(item: 0, section: 0)) as? UICollectionViewListCell)?.accessories.count ?? 0
    }
}
