...
extension WebInspector {
    struct NetworkBodyPreviewView: View {
        ...
        var body: some View {
            ...
            Group {
                switch selectedMode {
                case .text:
                    NetworkTextPreviewView(text: previewData.text, summary: bodyState.summary)
                case .json:
                    NetworkJSONPreviewView(nodes: previewData.jsonNodes ?? [])
                }
            }
                ...
                ToolbarItem(placement: .secondaryAction) {
                    if bodyState.canFetchBody {
                        Button {
                            ...
                            Task {
                                await inspector.fetchBodyIfNeeded(for: entry, body: bodyState, force: true)
                            }
                        } label: {
                            Label {
                                Text(LocalizedStringResource("network.body.fetch", bundle: .module))
                            } icon: {
                                if bodyState.fetchState == .fetching {
                                    ProgressView()
                                } else {
        ...
        private func previewModes(from previewData: NetworkBodyPreviewData) -> [PreviewMode] {
...
private struct NetworkTextPreviewView: View {
    let text: String?
    let summary: String?
    ...
    var body: some View {
        ScrollView {
            if let text {
                ...
                Text(LocalizedStringResource("network.body.unavailable", bundle: .module))
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .font(.subheadline)
...
private struct NetworkJSONPreviewView: View {
    let nodes: [NetworkJSONNode]

    var body: some View {
        if nodes.isEmpty {
            NetworkTextPreviewView(text: nil, summary: nil)
        } else {
            List {
                OutlineGroup(nodes, children: \.children) { node in
                    NetworkJSONRowView(node: node)
...
private struct NetworkJSONRowView: View {
    let node: NetworkJSONNode

    var body: some View {
        HStack(alignment: .top, spacing: 8) {
            NetworkJSONTypeBadge(symbol: badgeSymbol, tint: badgeTint)
            HStack(spacing: 4) {
                if !node.key.isEmpty {
                    Text(node.key)
...
private struct NetworkJSONTypeBadge: View {
    let symbol: String
    let tint: Color
