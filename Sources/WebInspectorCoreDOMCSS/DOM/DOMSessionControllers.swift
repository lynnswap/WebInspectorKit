...
final class DOMSessionDeleteUndoController {
    ...
    func stateIsCurrent(
        ...
        operation: String,
        recordError: (InspectorError?) -> Void
    ) -> Bool {
        guard currentDocumentID == state.documentID else {
            clear(using: undoManager, undoTarget: undoTarget)
            recordError(InspectorError("DOM document changed before \(operation)."))
            return false
        }
        return true
    }
    ...
    func updateDocumentID(
        ...
        guard let currentDocumentID else {
            clear(using: undoManager, undoTarget: undoTarget)
            recordError(InspectorError("DOM document is unavailable after delete undo operation."))
            return
        }
