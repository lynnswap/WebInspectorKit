...
struct NetworkSecurityViewControllerTests {
    ...
    @Test
    func modeControlUsesScrollableTabsAndOwnsRevealAndAccessibility() throws {
        let controller = NetworkDetailModeControlController(initialMode: .headers)
        let host = UIViewController()
        controller.view.frame = CGRect(x: 0, y: 0, width: 180, height: 44)
        ...
        #expect(controller.usesScrollablePickerForTesting)
        let scrollView = try #require(controller.scrollViewForTesting)
        #expect(scrollView.contentSize.width > scrollView.bounds.width)
        #expect(controller.segmentControlsForTesting.count == NetworkDetailViewController.Mode.allCases.count)
        let accessibilityElements = controller.modeAccessibilityElementsForTesting
        #expect(accessibilityElements.map(\.accessibilityLabel) == NetworkDetailViewController.Mode.allCases.map(\.title))
        #expect(accessibilityElements[0].accessibilityTraits.contains(.selected))

        controller.render(mode: .security, isEnabled: true)
        controller.view.layoutIfNeeded()
        #expect(scrollView.contentOffset.x > 0)
        #expect(accessibilityElements[3].accessibilityTraits.contains(.selected))
        ...
        #expect(accessibilityElements[3].accessibilityActivate())
        controller.view.layoutIfNeeded()
        #expect(selectedModes == [.security])
        #expect(accessibilityElements[3].accessibilityTraits.contains(.selected))
        #expect(scrollView.contentOffset.x > 0)

        controller.render(mode: .security, isEnabled: false)
        #expect(accessibilityElements.allSatisfy { $0.accessibilityTraits.contains(.notEnabled) })
        #expect(accessibilityElements[0].accessibilityActivate() == false)
        #expect(selectedModes == [.security])
    }

    @Test
    func adaptiveModeControlFallbackUsesMenuForCompactAndAccessibilitySizes() {
        let controller = NetworkDetailAdaptiveModeControlContent()
        let host = UIViewController()
        host.view.addSubview(controller.view)
        let window = showInWindow(host)
        defer { window.isHidden = true }
