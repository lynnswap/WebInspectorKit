...
@MainActor
struct NetworkEntryDetailViewControllerTests {
    @Test
    func detailShowsEmptyStateWithoutSelection() {
        let inspector = WINetworkModel(session: NetworkSession())
        let viewController = NetworkEntryDetailViewController(inspector: inspector)
    ...
    func detailDataSourceDoesNotRetainViewController() async {
        let inspector = WINetworkModel(session: NetworkSession())
        weak var weakViewController: NetworkEntryDetailViewController?
        var viewController: NetworkEntryDetailViewController? = NetworkEntryDetailViewController(
            inspector: inspector
        )
    ...
    func detailModeMenuReflectsBodyAvailability() async throws {
        ...
        #expect(menuActionTitles(in: viewController.modeMenuForTesting) == NetworkEntryDetailMode.allCases.map(\.title))
