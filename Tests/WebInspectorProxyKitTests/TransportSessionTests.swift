...
func targetReplyCarriesPerDomainSequenceWatermarks() async throws {
    ...
    await receiveTargetDispatch(
        session,
        targetID: .init("page-main"),
        message: #"{"method":"Network.requestWillBeSent","params":{"requestId":"request-1","frameId":"main-frame","loaderId":"main-loader","request":{"url":"https://example.com/"},"initiator":{"type":"other"},"timestamp":1}}"#
    )
    await receiveTargetDispatch(
        session,
        targetID: .init("page-main"),
