...
struct NetworkDetailViewControllerTests {
    ...
    func detailShowsEmptyStateWithoutSelection() {
        let model = V2_NetworkPanelModel(network: NetworkSession())
        let viewController = V2_NetworkDetailViewController(model: model)
    ...
    func responseBodyModeRequestsRuntimeFetchWhenBodyIsAvailable() async throws {
        ...
        let model = V2_NetworkPanelModel(network: network) { id in
    ...
    func compactContainerPushesAndPopsDetailFromSelection() async throws {
        ...
        let model = V2_NetworkPanelModel(network: network)
        let listViewController = V2_NetworkListViewController(model: model)
        let detailViewController = V2_NetworkDetailViewController(model: model)
        let navigationController = V2_NetworkCompactNavigationController(
            model: model,
