...
@Test
@MainActor
func loadingFinishedRequestHeadersMetricsAreDisplayAffectingChanges() throws {
    let session = NetworkSession()
    let targetID = ProtocolTarget.ID("page")
    let requestID = NetworkRequest.ProtocolID("0.metrics")
    let key = session.applyRequestWillBeSent(
        targetID: targetID,
        requestID: requestID,
        frameID: .init("main-frame"),
        ...
        request: .init(url: "https://media.example.test/segment.part"),
        resourceType: .other,
        timestamp: 1
    )
    let displayRevisionAfterRequest = session.requestDisplayRevision
    ...
    session.applyLoadingFinished(
        ...
        requestID: requestID,
        timestamp: 2,
        metrics: .init(requestHeaders: [
            "Range": "bytes=10-19",
        ])
    )

    let displayChanges = session.requestDisplayChanges(after: displayRevisionAfterRequest)
    #expect(displayChanges.revision == session.requestDisplayRevision)
    #expect(displayChanges.changedRequestIDs == [key])
    #expect(displayChanges.requiresFullReconcile == false)
}
